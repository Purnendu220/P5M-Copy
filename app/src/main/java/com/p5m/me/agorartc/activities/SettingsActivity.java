...
import com.p5m.me.R;
import com.p5m.me.agorartc.ui.ResolutionAdapter;
...
//        mPref.edit().putBoolean(AppConstants.PREF_ENABLE_STATS,
//                mVideoStatCheck.isActivated()).apply();
    ...
    private void saveVideoMirrorMode(String key, int value) {
        if (TextUtils.isEmpty(key))
            return;
        switch (key) {
            case AppConstants.PREF_MIRROR_LOCAL:
                config().setMirrorLocalIndex(value);
                break;
            case AppConstants.PREF_MIRROR_REMOTE:
                config().setMirrorRemoteIndex(value);
                break;
            case AppConstants.PREF_MIRROR_ENCODE:
